...
bool payloadEq(const Tuple& t, std::string_view b) {
    ...
    for (std::size_t i = 0; i < t.size(); ++i) {
        if (static_cast<char>(t.data()[i]) != b[i]) return false;
    }
    return true;
}
...
protected:
    void SetUp() override {
        ...
        // because CatalogManager just allocates a raw page and doesn't format it as a TableHeap.
        // Wait, TableHeap::create allocates its own page and it might be page 2 or 3.
        // We just need to make sure the root page is correct for SeqScanExecutor.
        // But TableInfo properties are const. We can just create TableHeap first!
    }
...
TEST_F(SeqScanExecutorTest, EmptyTable) {
    ...
    TableInfo info(table_info_->getTableId(), table_info_->getTableName(), heap->getFirstPageId(), table_info_->getIndexRootPage(), table_info_->getSchema());
