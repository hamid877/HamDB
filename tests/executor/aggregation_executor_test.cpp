...
TEST_F(AggregationExecutorTest, BasicAggregation) {
    ...
    agg_exprs.push_back(std::make_unique<ConstantExpression>(Value(1))); // COUNT(*) doesn't actually evaluate it this way, but we pass something
    ...
    AggregationExecutor agg(std::move(mock_child), std::move(group_bys), std::move(agg_exprs), agg_types, out_schema);
    agg.init();
    ...
    while (agg.next(&tuple, &rid)) {
        results.push_back({
            out_col0.evaluate(tuple, out_schema).getAsInteger(),
