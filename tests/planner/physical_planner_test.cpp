...
TEST_F(PhysicalPlannerTest, PlanSelectWithFilterAndLimit) {
    ...
    auto logical_plan = planner_->plan(std::move(stmt));
    auto plan = physical_planner_->plan(std::move(logical_plan));
    
    ASSERT_EQ(plan->getType(), PhysicalPlanType::LIMIT);
    ASSERT_EQ(plan->getChildren().size(), 1);
    
    auto proj = plan->getChildren()[0].get();
    ASSERT_EQ(proj->getType(), PhysicalPlanType::PROJECTION);
    ASSERT_EQ(proj->getChildren().size(), 1);
    
    auto filter = proj->getChildren()[0].get();
    ASSERT_EQ(filter->getType(), PhysicalPlanType::FILTER);
    ASSERT_EQ(filter->getChildren().size(), 1);
    
    auto seq = filter->getChildren()[0].get();
    ASSERT_EQ(seq->getType(), PhysicalPlanType::SEQ_SCAN);
    
    auto seq_scan = static_cast<SeqScanPlan*>(seq);
    ASSERT_EQ(seq_scan->getTableName(), "test_table");
}

TEST_F(PhysicalPlannerTest, PlanInsertValues) {
    ...
    ASSERT_EQ(plan->getType(), PhysicalPlanType::INSERT);
    ...
    ASSERT_EQ(values->getType(), PhysicalPlanType::VALUES);
    ASSERT_EQ(static_cast<ValuesPlan*>(values)->getValues().size(), 1);
}
