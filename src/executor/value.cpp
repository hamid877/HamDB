...
TypeId Value::getType() const {
    if (std::holds_alternative<int32_t>(value_)) return TypeId::Integer;
    if (std::holds_alternative<bool>(value_)) return TypeId::Boolean;
    if (std::holds_alternative<std::string>(value_)) return TypeId::Varchar;
    return TypeId::Null;
}
...
Value Value::compareEquals(const Value& other) const {
    if (isNull() || other.isNull()) return Value();
    if (getType() != other.getType()) return Value(false);
    return Value(value_ == other.value_);
}

Value Value::compareNotEquals(const Value& other) const {
    if (isNull() || other.isNull()) return Value();
    if (getType() != other.getType()) return Value(true);
    return Value(value_ != other.value_);
}

Value Value::compareLessThan(const Value& other) const {
    if (isNull() || other.isNull()) return Value();
    if (getType() != other.getType()) throw std::runtime_error("Type mismatch in comparison");
    return Value(value_ < other.value_);
}
