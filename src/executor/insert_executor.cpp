...
InsertExecutor::InsertExecutor(ExecutorContext* exec_ctx, const TableInfo* table_info, std::unique_ptr<AbstractExecutor> child_executor)
...
bool InsertExecutor::next(Tuple* tuple, RID* rid) {
    ...
    while (child_executor_->next(&child_tuple, &child_rid)) {
        ...
        LogRecord log(txn->getTransactionId(), LogRecordType::INSERT, kInvalidLSN, new_rid, child_tuple);
        log_mgr->append(log);
