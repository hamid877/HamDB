...
void AggregationHashTable::insertCombine(const AggregateKey& agg_key, const AggregateValue& agg_val) {
...
std::vector<AggregateState> AggregationHashTable::initializeState(const AggregateValue& agg_val) const {
...
void AggregationHashTable::combineState(std::vector<AggregateState>& states, const AggregateValue& agg_val) const {
    for (size_t i = 0; i < agg_types_.size(); ++i) {
        const auto& val = agg_val.aggregates_[i];
        switch (agg_types_[i]) {
            case AggregateType::CountStar:
            ...
            case AggregateType::Min:
                if (!val.isNull()) {
                    if (states[i].value_.isNull() || val.compareLessThan(states[i].value_).getAsBoolean()) {
                        states[i].value_ = val;
                    }
                }
                break;
            case AggregateType::Max:
                if (!val.isNull()) {
                    if (states[i].value_.isNull() || val.compareGreaterThan(states[i].value_).getAsBoolean()) {
                        states[i].value_ = val;
                    }
                }
                break;
