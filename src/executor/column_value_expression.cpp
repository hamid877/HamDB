...
namespace hamdb
{
    ...
    Value ColumnValueExpression::evaluate(const Tuple& tuple, const Schema& schema) const
    {
        ...
        for (uint32_t i = 0; i <= col_idx_; ++i)
        {
            ...
            if (i == col_idx_)
            {
                ...
                {
                    int32_t val = 0;
                    if (deserializer.readInt32(val) != Status::Ok)
                ...
                {
                    bool val = false;
                    if (deserializer.readBool(val) != Status::Ok)
                        throw std::runtime_error("Deserialization failed");
                    return Value(val);
                }
                ...
                {
                    std::string val;
                    if (deserializer.readString(val) != Status::Ok)
                        throw std::runtime_error("Deserialization failed");
    ...
    Value ColumnValueExpression::evaluateJoin(const Tuple* left_tuple, const Schema* left_schema,
