...
UpdateExecutor::UpdateExecutor(ExecutorContext* exec_ctx, const TableInfo* table_info, std::unique_ptr<AbstractExecutor> child_executor, std::vector<std::unique_ptr<Expression>> target_expressions)
    : exec_ctx_(exec_ctx), table_info_(table_info), child_executor_(std::move(child_executor)), target_expressions_(std::move(target_expressions)) {
...
bool UpdateExecutor::next(Tuple* tuple, RID* rid) {
    ...
    while (child_executor_->next(&child_tuple, &child_rid)) {
        ...
        if (bplus_tree_) {
            int64_t old_key = ColumnValueExpression(0).evaluate(child_tuple, table_info_->getSchema()).getAsInteger();
        ...
        LogRecord log(txn->getTransactionId(), LogRecordType::UPDATE, kInvalidLSN, child_rid, child_tuple, new_tuple);
        log_mgr->append(log);
