...
namespace hamdb
{
    ...
    AggregationExecutor::AggregationExecutor(std::unique_ptr<AbstractExecutor> child,
        ...
        : child_(std::move(child)), group_bys_(std::move(group_bys)),
          aggregates_(std::move(aggregates)), agg_types_(std::move(agg_types)),
          output_schema_(std::move(output_schema)), aht_(agg_types_)
    {
    }
    ...
    bool AggregationExecutor::next(Tuple* tuple, RID* rid)
    {
        ...
        if (!is_successful_)
        {
            ...
            for (size_t i = 0; i < agg_types_.size(); ++i)
            {
                ...
                case AggregateType::Avg:
                    // Just put 0 for NULLs since we lack NULL serialization support in this basic
                    // version
                    projected_values.emplace_back(static_cast<int32_t>(0));
                    break;
                }
        ...
        for (size_t i = 0; i < projected_values.size(); ++i)
        {
            ...
            {
                bool val =
                    projected_values[i].isNull() ? false : projected_values[i].getAsBoolean();
                (void)ser.writeBool(val);
            }
            else if (type == ColumnType::Varchar)
            {
                std::string val =
                    projected_values[i].isNull() ? "" : projected_values[i].getAsVarchar();
