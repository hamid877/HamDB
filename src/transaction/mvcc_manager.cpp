...
bool MvccManager::insert(Transaction* txn,
                         const RID& rid,
                         std::vector<std::byte> data)
{
    ...
    if (it != chains_.end()) {
        ...
        if (head != nullptr && !head->is_deleted) {
            ...
            if (head->begin_txn_id == txn->getTransactionId() &&
                !head->is_committed) {
                return false;
            }
    ...
    auto version = std::make_shared<TupleVersion>(
...
bool MvccManager::update(Transaction* txn,
                         const RID& rid,
                         std::vector<std::byte> new_data)
{
    ...
    auto new_version = std::make_shared<TupleVersion>(
        txn->getTransactionId(),
        std::move(new_data),
        false,
...
bool MvccManager::remove(Transaction* txn, const RID& rid)
{
    ...
    auto tombstone = std::make_shared<TupleVersion>(
        txn->getTransactionId(),
        std::vector<std::byte>{},
        true,
        head);
