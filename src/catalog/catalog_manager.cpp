...
namespace hamdb
{
    ...
    Status CatalogManager::createTable(const std::string& table_name, const Schema& schema,
                                       TableInfo*& out_info)
    {
        ...
        auto table_info = std::make_unique<TableInfo>(next_table_id_++, table_name, heap_root_page,
                                                      index_root_page, schema);
    ...
    Status CatalogManager::saveToDisk()
    {
        ...
        if (auto status = serializer.writeUInt32(next_table_id_); status != Status::Ok)
            return status;
        if (auto status = serializer.writeUInt32(static_cast<std::uint32_t>(tables_.size()));
            status != Status::Ok)
            return status;

        for (const auto& [name, info] : tables_)
        {
            if (auto status = info->serialize(serializer); status != Status::Ok)
                return status;
        }
